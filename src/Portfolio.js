...
async function fetchSellAdvice({ symbol, name, ind, avgPrice, shares, currency }) {
  ...
  const prompt = `あなたは技術指標の解説者です。投資アドバイザーではなく、投資助言・投資勧誘は行いません。以下の保有銘柄について「売り時」と「空売り機会」の観点で客観的なデータ分析を提供してください。
...
// ── API ヘルパー ─────────────────────────────────────────────────
async function apiFetchHoldings() {
...
async function apiAddHolding(symbol, name, shares, avgPrice) {
  const res = await fetch('/api/portfolio', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ symbol, name, shares, avg_price: avgPrice }),
  });
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  return res.json();
}
...
export default function Portfolio({ kabuStatus }) {
  ...
  const addHolding = async () => {
    ...
    try {
      await apiAddHolding(sym, sym, shares, avgPrice);
      const updated = await apiFetchHoldings();
      setHoldings(updated);
      setForm({ symbol: '', shares: '', avgPrice: '' });
      setFormErr('');
    } catch (e) {
      setFormErr(`追加に失敗しました: ${e.message}`);
    }
  };
